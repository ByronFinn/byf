  ...
  describe('explicit markers (legacy compatibility)', () => {
    describe('no markers', () => {
      ...
      it('hashes the content correctly', () => {
        ...
        expect(fingerprint(defined(plan.blocks[0]).text)).toBe(fingerprint(prompt));
      });
...
Session`;
  ...
  describe('hash stability and uniqueness', () => {
    it('produces same hash for identical content', () => {
      ...
      expect(fingerprint(defined(plan1.blocks[0]).text)).toBe(
        fingerprint(defined(plan2.blocks[0]).text),
      );
    });
    ...
    it('produces different hashes for different content', () => {
      ...
      expect(fingerprint(defined(plan1.blocks[0]).text)).not.toBe(
        fingerprint(defined(plan2.blocks[0]).text),
      );
    });
...
Session`;
      ...
      for (let i = 0; i < plan1.blocks.length; i++) {
        expect(fingerprint(defined(plan1.blocks[i]).text)).toBe(
          fingerprint(defined(plan2.blocks[i]).text),
        );
        expect(defined(plan1.blocks[i]).name).toBe(defined(plan2.blocks[i]).name);
        expect(defined(plan1.blocks[i]).cacheScope).toBe(defined(plan2.blocks[i]).cacheScope);
      }
    });
