...
function localDate(y: number, monthIndex: number, d: number, h = 0, m = 0, s = 0): number {
...
describe('jitteredNextCronRunMs — recurring', () => {
  ...
  it('respects BYF_CRON_NO_JITTER=1 — no offset', () => {
    const prev = process.env['BYF_CRON_NO_JITTER'];
    process.env['BYF_CRON_NO_JITTER'] = '1';
    try {
      const parsed = parseCronExpression('*/5 * * * *');
      const ideal = localDate(2024, 5, 1, 12, 5, 0);
      const jittered = jitteredNextCronRunMs(
      ...
      if (prev === undefined) delete process.env['BYF_CRON_NO_JITTER'];
      else process.env['BYF_CRON_NO_JITTER'] = prev;
    }
  });
});
...
describe('oneShotJitteredNextCronRunMs', () => {
  ...
  it('respects BYF_CRON_NO_JITTER=1', () => {
    const prev = process.env['BYF_CRON_NO_JITTER'];
    process.env['BYF_CRON_NO_JITTER'] = '1';
    try {
      const ideal = localDate(2024, 5, 1, 14, 0, 0);
...
// suite (in case a future test in the same bun:test worker reads it).
beforeEach(() => {
