 ...
 * `ClockSources`; `BYF_CRON_NO_JITTER=1` pins fire counts on the
 * recurring tests.
 */
...
const ORIGINAL_ENV_NO_JITTER = process.env['BYF_CRON_NO_JITTER'];
...
describe('createCronScheduler — tick behaviour', () => {
  ...
  afterEach(() => {
    if (ORIGINAL_ENV_NO_JITTER === undefined) {
      ...
      process.env['BYF_CRON_NO_JITTER'] = ORIGINAL_ENV_NO_JITTER;
    }
  });
  ...
  it('one-shot task fires once then is removed', () => {
    ...
    const taskId = defined(h.tasks[0]).id;
  ...
  it('only the due task fires when two tasks are scheduled', () => {
    ...
    const dueId = defined(h.tasks[0]).id;
...
describe('createCronScheduler — getNextFireForTask', () => {
  ...
  it('preserves pending jittered slot when ideal is just past', () => {
    ...
    h.scheduler.tick();
    expect(h.fired).toHaveLength(1);
    const firedAtIdeal = defined(h.fired[0]).task; // proves we delivered
