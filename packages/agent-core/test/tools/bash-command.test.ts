...
describe('bash-command parse — 单子命令分类', () => {
  ...
  it('git 按子命令分类读写；git 整体不可收窄（触碰 .git）', () => {
    const status = defined(parseBashCommand('git status').subcommands[0]);
    expect(status.kind).toBe('broad');
    const add = defined(parseBashCommand('git add src/foo.ts').subcommands[0]);
  ...
  it('build/test 动词宽化为 broad（带文件参数也收窄失败——会加载任意依赖）', () => {
    ...
    const withFile = defined(parseBashCommand('bun test test/foo.test.ts').subcommands[0]);
  ...
  it('curl URL 为 broad；curl -o 精确落盘 → narrow write', () => {
    ...
    const o = defined(parseBashCommand('curl -o out.html https://x.com').subcommands[0]);
  ...
  it('解释器：-c/-e 内层代码为已知绕过面 → indirect；脚本文件 → broad', () => {
    ...
    const script = defined(parseBashCommand('python setup.py').subcommands[0]);
...
describe('bash-command parse — review 修复（& / 换行 / find -exec / 深度上限）', () => {
  ...
  it('bash -c 深层嵌套超限转 indirect（防栈溢出 DoS）', () => {
    ...
    let result: ReturnType<typeof parseBashCommand> | undefined;
    expect(() => {
