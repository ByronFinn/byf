...
describe('createRPC', () => {
  ...
  it('binds prototype methods and rehydrates plain remote errors as ByfError(internal)', async () => {
    ...
    await expect(hostProxy.fail({ code: 'boom' })).rejects.toBeInstanceOf(ByfError);
    expect(hostImpl.approvals).toEqual(['approval-2']);
  });

  it('passes a thrown ByfError across the wire preserving code and details', async () => {
    ...
    expect(received).toBeInstanceOf(ByfError);
    const error = received as ByfError;
  ...
  it('serializes remote error details through JSON semantics before crossing the wire', async () => {
    ...
    await connectRemote({
      async fail() {
        ...
        throw new ByfError(ErrorCodes.INTERNAL, 'Remote failed', {
          details,
        });
      },
    });
  ...
  it('rehydrates provider.* codes as ByfError', async () => {
    interface RemoteSide {
    ...
    await connectRemote({
      async callProvider() {
        throw new ByfError(ErrorCodes.PROVIDER_RATE_LIMIT, 'Upstream rate limit', {
  ...
  it('collapses non-Error throws to ByfError(internal)', async () => {
    interface RemoteSide {
    ...
    await expect(remoteProxy.misbehave({})).rejects.toBeInstanceOf(ByfError);
  });
});
