...
async function makeProject(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'byf-workspace-local-'));
  tempDirs.push(root);
...
async function expectConfigInvalid(promise: Promise<unknown>, message: string): Promise<void> {
  await expect(promise).rejects.toBeInstanceOf(ByfError);
...
describe('workspace local config', () => {
  it('returns empty workspace config when local.toml is missing', async () => {
    ...
    await expect(
      ...
      projectRoot: root,
      configPath: join(root, '.byf', 'local.toml'),
      additionalDirs: [],
    });
  });
  ...
  it('loads additional_dir array from the project root when started nested', async () => {
    ...
    await mkdir(join(root, '.byf'), { recursive: true });
    await writeFile(
      join(root, '.byf', 'local.toml'),
  ...
  it('appends multiple directories and deduplicates normalized paths', async () => {
    ...
    const configPath = join(root, '.byf', 'local.toml');
