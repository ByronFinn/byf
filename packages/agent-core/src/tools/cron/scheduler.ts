 ...
 *     stderr trace gated on `BYF_CRON_DEBUG=1`) so one busted cron
...
export interface CronSchedulerOptions {
   ...
   * `byf resume` does not replay the fire.
   *
   ...
   *     manually.
   *
   * Used by P1.8 to wire `BYF_CRON_MANUAL_TICK=1` to disable the
   * timer.
   */
...
export function createCronScheduler(opts: CronSchedulerOptions): CronScheduler {
  ...
  // Now persisted across `byf resume` via `task.lastFiredAt`: when
    ...
    try {
      ...
      const persistedCursor =
        ...
        task.lastFiredAt <= clocks.wallNow()
          ? task.lastFiredAt
          : undefined;
      const cursor = seen ?? persistedCursor;
