/**
 * WorkspaceRegistry：`<byf-home>/workspaces.json` 的所有者（PRD-0035 R-A6、
 * ADR-0037 D3）。
 *
 * 由 `apps/web/server/src/workspace-registry.ts` 上移：Web 与未来 TUI 共用
 * 同一实现。/grill 决议：丢弃历史 string[] 格式兼容，只保留 order+hidden
 * 新结构；文件不存在或损坏/非新结构视为空注册表（首次写入覆盖为新结构）。
 */
 ...
 *  会话)在此登记;会话索引里出现但未登记的 workDir 由调用方经
 *  `listIndexedWorkDirs` 补入枚举结果。写为原子替换,损坏时视为空表。 */
export class WorkspaceRegistry {
  ...
  private async read(): Promise<WorkspaceRegistryData> {
    let raw: string;
    try {
    ...
    try {
      ...
      // 旧 string[] 格式与非对象结构:按 /grill 决议视为空注册表。
      return { order: [], hidden: [] };
    } catch {
      return { order: [], hidden: [] };
    }
  ...
  private async write(data: WorkspaceRegistryData): Promise<void> {
...
export async function listIndexedWorkDirs(homeDir: string): Promise<string[]> {
  const index = await readSessionIndex(homeDir, join(homeDir, 'sessions'));
  const seen = new Set<string>();
  const dirs: string[] = [];
  for (const entry of index.values()) {
    const workDir = entry.workDir;
    if (workDir.length === 0 || seen.has(workDir)) continue;
    seen.add(workDir);
    dirs.push(workDir);
  }
  return dirs;
...
export async function findSessionWorkDir(
  ...
  const index = await readSessionIndex(homeDir, join(homeDir, 'sessions'));
  return index.get(sessionId)?.workDir;
}
