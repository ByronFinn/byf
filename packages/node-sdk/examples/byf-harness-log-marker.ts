...
Options:
  -s, --session <id>   Existing session id to resume and mark
      --home <dir>     Byf home dir; defaults to BYF_HOME or ~/.byf
...
async function main(): Promise<void> {
  ...
  const resolvedHome = resolveByfHome(options.homeDir);
  const harness = new ByfHarness({
    identity: { userAgentProduct: 'byf-cli', version: 'log-marker' },
  ...
  if (!sessionMatched) {
    process.stderr.write(
      [
        ...
        'Check that BYF_LOG_LEVEL is not "off" and that the session id exists in this BYF_HOME.',
        '',
      ].join('\n'),
    );
    process.exitCode = 1;
