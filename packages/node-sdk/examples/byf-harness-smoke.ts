...
async function main(): Promise<void> {
  const workDir = await mkdtemp(join(tmpdir(), 'byf-harness-smoke-'));
  const homeDir = await mkdtemp(join(tmpdir(), 'byf-harness-home-'));
  ...
  try {
    ...
    const model = config.defaultModel ?? 'byf/byf-for-coding';
