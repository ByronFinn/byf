...
async function main(): Promise<void> {
  ...
  process.env['BYF_LOG_SESSION_MAX_BYTES'] = String(MAX_LOG_BYTES);
  process.env['BYF_LOG_SESSION_FILES'] = '2';
  process.env['BYF_LOG_GLOBAL_MAX_BYTES'] = String(MAX_LOG_BYTES);
  process.env['BYF_LOG_GLOBAL_FILES'] = '2';
  ...
  const harness = new ByfHarness({
    identity: { userAgentProduct: 'byf-cli', version: '0.1.1' },
    homeDir: TEST_HOME,
  });
