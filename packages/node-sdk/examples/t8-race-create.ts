...
const identity: any = { userAgentProduct: 'byf-cli', version: '0.0.1-test' };
const harnessA = new ByfHarness({ identity, homeDir });
const harnessB = new ByfHarness({ identity, homeDir });

async function run(label: string, h: ByfHarness): Promise<void> {
  try {
