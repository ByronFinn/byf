...
describe('e2e: streaming fidelity', () => {
  ...
  describe('ToolCall argument assembly from ToolCallPart deltas', () => {
    it('single ToolCall with 10+ ToolCallPart deltas assembles arguments correctly', async () => {
      ...
      // `ToolCall['arguments']` may legally be null, so narrow on the real
      // requirement (a string to parse) instead of a null-only check.
      ...
      const parsed = JSON.parse(firstArgs) as Record<string, unknown>;
