...
- **Debugged by**: `/debug`（2026-08-19，第四轮）— 抽屉与轨道图打磨：① DetailsDrawer 为 fixed 浮层、展开时中间栏不回流（页面不协调）——宽度状态上移至 details-context，AppFrame 展开时经 `resolveDrawerPush` 从网格预留第三列宽度，center 自动让位、窄屏（center < 360）退回浮层，拖宽期间关闭过渡逐帧同步；② 缺 deepseek-harness 式 input/model/tools 横向轨道轨迹图——按用户要求**忠实移植 deepseek-harness `ui-trajectory` 的 TrajectoryTimeline**（MIT）：`lib/timeline.ts`（wire 折叠为轨迹账本 + sequence/duration/time/actual 四模式投影，三轨 Input/Model/Tools）、`TrajectoryTimeline.tsx` + CSS module（拖选聚焦区间/点击定位/滚轮缩放/右键平移/悬停 tooltip，ledger 区间外变暗），WireTab 工具栏加等宽↔真实时长切换。回归：timeline/columns 单测新增，97 客户端测试全绿，浏览器实测（时间轴三轨渲染、TTFT 渐变、时长切换 span 数 29→17）。
