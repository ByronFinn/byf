...
11. **CI 第一个红是计时，而正确的修法是不起进程**（三次运行才收敛：`35765202338` → `35766699956` → `35769431862`）。`apps/cli/test/scripts/compile/compile-entry-source.test.ts` 的 4 条用例各自 spawn 一次仓库钉住的 tsc，跑在 Bun 的 5000ms 默认上限上：本机一次冷启动 ~2s，CI 报 3.7s / 5.1s / 22.2s。第一步只把预算写清楚——单次 spawn 20s、单条用例 55s（严格高于两次 spawn，即 `.github/workflows/ci.yml` 记着的那条不变量）、整文件 100s 排在 harness 的 120s 单文件杀进程之下——并补 `assertTscRan`：Bun 用超时杀进程时仍报 `exitCode: 0`，而"没有语法诊断"这类**空断言**会把被杀的 tsc 读成通过，这正是它自己防的那种 vacuous pass。但**提高预算只是把失败从"无信息超时"换成"可归因的杀进程"**：第三次运行照旧在 20s 上限被打死三条。变量是 runner 争用，不是被测代码，所以没有能活下来的数字。最终改结构——不 spawn，用 `Bun.Transpiler`（`loader: 'ts'`），也就是 `bun compile` 消费这份 codegen 时用的同一个解析器；200 次 transform 实测 2ms，整个文件从 7s 落到亚毫秒，而且再没有"进程没起来"这种失败模式要防。随之失去的是脚手架而不是覆盖：`--allowImportingTsExtensions` / `--types bun` / `--typeRoots` 三个旗子只为让 tsc 肯看这个文件，TS2xxx 存活证明只为证明 spawn 发生过；"发布二进制真编得出来"由 CI 的 `Compile darwin-arm64 binary` 与 `test:native:smoke` 用真编译器负责。防"检查自身失效"的两条断言原地保留（已知坏形状必须被拒、真 codegen 追加坏尾必须被拒），并另用一次性脚本在仓库外验过：把当前 codegen 输出的 `(globalThis as Record<string, unknown>)[…]` 改回 `36064cf` 之前的 `(globalThis as any)NAME`，解析器确实拒绝——回归仍抓得住。
