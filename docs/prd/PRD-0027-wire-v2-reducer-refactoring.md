...
| PR-7  | Phase 6 | 删除 `restoring`/`RecordRestoreHandler`/`AgentRecords` + 统一 toEvent ✅ 已落地（goal.updated 事件含 live 瞬态（complete status、实时 wallClockMs），toEvent 的 post-apply state 无法表达 → 已决议：transient 车辆 op `goal.updated`（persist:false + apply identity + toEvent 从 payload 透传，对标 kimi `skill.activate`），8 个发射点统一走 dispatch，见 PR-8 待办落地） | 全量回归                                                            |
