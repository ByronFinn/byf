...
> **Status**: In Progress（Phase 1-6 完成，PR-2~PR-7 已落地；goal.updated 统一待办已落地） | **PRD**: PRD-0027 | **Created**: 2026-08-11 | **Last updated**: 2026-08-12
>
...
| PR-7  | Phase 6 | 删除 `restoring`/`RecordRestoreHandler`/`AgentRecords` + 统一 toEvent ✅ 已落地（goal.updated 事件含 live 瞬态（complete status、实时 wallClockMs），toEvent 的 post-apply state 无法表达 → 已决议：transient 车辆 op `goal.updated`（persist:false + apply identity + toEvent 从 payload 透传，对标 kimi `skill.activate`），7 个发射点统一走 dispatch，见 PR-8 待办落地） | 全量回归                                                            |
| PR-8+ | Phase 7 | transient op ✅ 已随 Phase 5 落地（`context.output_offloaded`/`context.pruning`）+ goal.updated 统一 ✅ 已落地（transient 车辆 op，见 PR-7）；manifest / checkpoint（可选，按需）                                                                                                                                                                                           | 独立                                                                |
...
- **Reviewed by**: `/review`（2026-08-12，PR #262）—— Test/Code/Impact 三视角并行评审，无矛盾。实现质量与测试覆盖均 Approve；阻塞项：① CI `fmt:check` 失败（已修，含 1 个本 PR 文件 + 3 个 dev 遗留）；② changeset 级别——`WireFoldHandlers` 删除 + fold helper 签名变更触及 agent-core barrel 导出，经用户确认为 **minor**（@byfriends/agent-core 未发布到 npm，wire-fold 唯一消费者 apps/vis 已在本 PR 同步，视为 monorepo-internal）。minor 清理一并并入：`seal()` 补 flush、context ops 去除冗余 cast、full-compaction 移除未消费的 `compactions` 状态、6 处 stale restoring 注释、新增 legacy-transient restore 回归测试。
- **Prototyped by**: `/have-a-try`（2026-08-12）—— 问题：「goal.updated 能否统一进 toEvent？」。验证结论：`goal.update` 挂 toEvent 不可行（N3 计步事件泄漏 + complete overlay 丢失，post-apply state 无法表达 live 瞬态）；transient 车辆 op（`persist:false` + identity apply + toEvent 从 payload 透传）机械上完全可行——live snapshot 完整携带、journal 零落盘、restore 静默、N3 不受影响。与 kimi `skill.activate`（`skillOps.ts`，op 只为派生事件而存在）官方模式一致。据此实现 Phase 6 待办（goal.updated 统一）。
- **Parent Issue**: #260
