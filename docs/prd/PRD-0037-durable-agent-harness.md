...
- **Sliced into**:
  ...
  - #328 — [PRD-0037] AgentLane 完整 API + lane CRUD + per-lane 配置 — Done (AFK, blocked by #325)
  - #329 — [PRD-0037] navigateTree + branch summary + 手动/自动压缩操作 — Done (AFK, blocked by #328)
  - #330 — [PRD-0037] fork 重写 + 确定性子会话 id + 子代理迁移 — Done (AFK, blocked by #328)
  - #331 — [PRD-0037] goal 映射：custom entries + before_run_end followUp — Done (AFK, blocked by #325)
  - #332 — [PRD-0037] v2 hooks 目录 + 重放矩阵 + 持久化输出 — Done (AFK, blocked by #325)
  - #333 — [PRD-0037] shell hooks 13 事件映射桥接 — Done (AFK, blocked by #332)
  - #334 — [PRD-0037] v2 events 目录 + watch()/watchSession() 缓冲订阅 — Done (AFK, blocked by #323)
  - #335 — [PRD-0037] kosong deferred：StopReason + DeferredHandle + fetchDeferred — Done (AFK，无阻塞，可先行)
  - #336 — [PRD-0037] harness Park 挂起/兑换闭环 — Done (AFK, blocked by #325, #335)
  - #337 — [PRD-0037] packages/storage SQLite 后端 + branch cache + leases — Done (AFK, blocked by #319, #326)
  - #338 — [PRD-0037] telemetry span 树 + 默认脱敏 — Done (AFK, blocked by #323)
  - #339 — [PRD-0037] CLI TUI 迁 v2 引擎 + suspended 启动 UX + 默认切换 — Partial/Stop-loss（HITL 人审待做；v2 dogfood 入口/事件目录/恢复 API 就绪，默认切换待 host 面移植后）(HITL, blocked by #326, #327, #331, #333, #334)
  - #340 — [PRD-0037] web 迁 watch 快照+事件模型 + DTO 重定义 — Partial/Stop-loss（D2 已修：busy 深度计数覆盖全部 agent；watch 迁移与 DTO 人审待做）(HITL, blocked by #328, #334)
  - #341 — [PRD-0037] headless 适配 + Inspector/检视 2.0 投影 — Partial（Inspector 2.0 投影完成；-p v2 模式待 v2 RPC 面）(AFK, blocked by #339)
  - #342 — [PRD-0037] 契约收尾：删除 wire 1.1 + 旧引擎 + AGENTS.md/CONTEXT.md 定稿 — Blocked（待 #339-#341 迁移完成）(AFK, blocked by #339, #340, #341)
