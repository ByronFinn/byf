...
- **Sliced into**（3 个纵切，#250 先行、#251/#252 并行）:
  - #250 — pi-tui 升级 0.74→0.80.6 + wire-scan turn_begin 修复（AFK，先行，无依赖）
  - #251 — 抽取 wire 折叠纯函数，消除核心/vis 投影重复（AFK，R3，与 #252 并行）
  - #252 — 恢复 restoreRecord exhaustive 守护 + live-only record 文档化（AFK，R4，与 #251 并行）
