...
> **Status**: Done | **PRD**: PRD-0025 | **Created**: 2026-07-13 | **Last updated**: 2026-07-14
>
> 父 Issue #249。
...
- **Parent Issue**: #249。
- **Domain terms added/updated**: CONTEXT.md「Wire Records」「输出卸载 (Output Offloading)」补充 live-only 语义；「wire 折叠 / 投影函数」对齐多导出 fold API + effect port 表述。
- **Sliced into**（3 个纵切，#250 先行、#251/#252 并行；已合入 `feat/prd-0025-wire-fold-pure-fn`）:
...
- **Review follow-up (2026-07-14)**: 闭合首轮 `/review` 缺口——8 子系统 `isAgentRecordOfPrefix` 穷尽 switch；`foldApplyCompaction` 共享；内核/纯 fold/vis 三方一致性测试（空 tool、真 partial compaction）；死 `_Check` 改为强制求值；live-only no-op 行为测试；CONTEXT 对齐。
- **Reviewed by**: `/review` 复审 2026-07-14 — Test / Code / Impact 均 Approve。
