...
- **Debugged by**: `/debug` (2026-06-18) — 修复 `SubagentsListApp`/`SubagentLiveViewer` 的 frame 错位：用 `@earendil-works/pi-tui` 的 ANSI 感知 helper 替换本地 ANSI 计数的 `visibleWidth`，并将 header/body 填充到全终端宽度。
- **Debugged by**: `/debug` (2026-06-18) — 修复 `/agent` 浏览器卡死/无响应：在 `SubagentsController` 中补充列表轮询更新和 live-viewer 快照更新后缺失的 `FullscreenHost.requestRender()` 调用。
- **Debugged by**: `/debug` (2026-06-18) — 修复列表层方向键选择时详情/预览不刷新的问题：添加 `onSelectionChange` 回调到 `SubagentsListApp`，将 `SubagentsController.pushListProps` 接入即时刷新；修复详情面板工具状态计数错误，将进行中的工具编码为 `selectedDetail.toolList` 中的 `… Name`；通过 `SubagentPreviewPane.activityLines` 为输出预览添加实时活动流。
- **Reviewed by**: `/review` (2026-06-18) — 修复 `ToolCallComponent` 快照监听器的单槽冲突：添加 `addSnapshotListener()`（多播）同时保留 `setSnapshotListener()` 向后兼容；更新 `AgentGroupComponent`/`ReadGroupComponent`/`SubagentsController` 分别退订。从 `/agent` 列表中过滤掉 `backgrounded` 代理（它们属于 `/tasks`）。移除未使用的 `prev` 变量并修正 `SubagentLiveViewer` 中 `renderBody` 的返回类型。
- **Debugged by**: `/debug` (2026-06-18) — 修复前台 sub-agent 流式输出 `assistant.delta`/`tool.call.delta` 时的流式冻结 + 乱码（"每行一个字符"、布局坍缩）。根因：`SubagentLiveViewer` 的快照订阅没有节流，每个 token 都触发完整的 `renderLines` + `requestRender`，压垮终端 diff 渲染器。修复：(1) 在 `SubagentsController` 中将快照回调合并为每 80ms 一次渲染（镜像 `AgentGroupComponent.THROTTLE_MS`），查看器关闭时取消待处理的定时器；(2) 提取 `sanitizeForDisplay`，在渲染前从流式文本/输出/错误中剥离原始 C0 控制字符（`\r`/`\b`/`\x07`/vt/ff）；(3) 使用 `wrapTextWithAnsi` 软换行长行文本替代硬截断。回归测试：合并 100 个 delta → 1 次渲染，控制字符剥离，软换行保留完整内容。
