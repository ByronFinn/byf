...
| M2 | vis DTO 双源维护且已漂移 | 方案 A：删副本 + `import type` 相对路径 |
...
5. web 经 `import type` 直接从 `../../../shared/types`（与 vis-server 现有做法一致）获取类型
...
- **M2 方案 A**：`shared/types.ts` 是纯类型模块（14 type/interface，0 运行时导出），`import type` 编译后擦除，不受 web tsconfig `include: ["src"]` 限制。当初复制副本的顾虑（避免拉 vis-server 源码进 web）经验证不成立。
