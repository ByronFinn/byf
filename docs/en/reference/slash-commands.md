...
| `/fork`                    | —           | Fork a new session from the current one, preserving the full conversation history. The copy covers the conversation only — it does not undo tool side effects, and `/rewind` is a fork to an earlier message, not a file rollback. | No               |
