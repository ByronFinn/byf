...
The most common entry point is to run `byf` directly to start a fresh session in the current directory:
...
This automatically finds and resumes the most recent session under the current working directory. To pick a different historical session, run `byf --session` to enter the interactive picker, or pass a known session ID directly:
...
Bundle a session into a ZIP file for sharing, archival, or bug reports. The exported archive contains all files under the session directory, such as context records, state files, and the session diagnostic log if that session has already produced `logs/byf.log`.
