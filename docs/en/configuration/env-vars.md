...
BYF uses environment variables to override default paths, switch OAuth endpoints, and adjust runtime behavior. Most variables are read when the `byf` process starts up; a few (such as the telemetry switch, the OAuth lock, and diagnostic logging) are read when the relevant subsystem initializes. BYF's own variables use the `BYF_*` prefix; in addition, the CLI also reads a number of standard system variables.

::: warning Note
**Provider credentials are not in this list**: key variables such as `BYF_API_KEY`, `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, and `GOOGLE_API_KEY` are **not** read automatically from `process.env`. They must be written into the `[providers.<name>]` section of `config.toml` (as `api_key` / `base_url`) or into the `[providers.<name>.env]` subtable; merely `export`ing them in your shell will not give a provider credentials automatically. See [Configuration overrides](./overrides.md#provider-credentials) and [Providers](./providers.md) for details.
:::

## Core paths

`BYF_HOME` overrides BYF's data root directory, defaulting to `~/.byf`. The CLI's own application data, `byf-core`'s config, the ripgrep cache, and OAuth credentials all land under this directory.
...
::: warning Note
Make sure the directory is writable once you set it. Multiple `byf` instances that share the same `BYF_HOME` will share both the config and credential files.
:::
...
::: warning Note
`BYF_HOSTED_BASE_URL` and the `BYF_BASE_URL` from the previous section are two different variables: the former targets the OAuth-logged-in hosted service and defaults to `byf.dev`; the latter targets providers that use a BYF API key directly and defaults to `example.com`. Distinguish them by use case.
:::
...
| `BYF_PRINT_WAIT_CEILING_S`          | Override `[background].print_wait_ceiling_s`: max seconds `byf -p` waits for background tasks after the main turn (and after goal/cron holds release). On timeout, print mode ends the wait and exits non-zero without killing tasks. Does **not** bound goal hold or session-cron keep-alive.                                                                                                                                   | Positive integer seconds; when unset, reads `config.toml`, then defaults to `3600`                                                         |
| `BYF_DISABLE_CRON`                  | Kill-switch for session-scoped cron: when set to `1`, `CronCreate` fails and the scheduler does not fire tasks                                                                                                                                                                                                                                                                                                                   | Exactly `1`                                                                                                                                |
| `BYF_CRON_NO_JITTER`                | Disable deterministic per-task fire jitter (useful in tests/benches). Production should leave this unset.                                                                                                                                                                                                                                                                                                                        | Exactly `1`                                                                                                                                |
...
`BYF_PRINT_WAIT_CEILING_S` likewise overrides `print_wait_ceiling_s`. Example for a shorter CI budget:
...
`BYF_WSL_CLIPBOARD_IMAGE_PATH` is injected automatically by the CLI when it spawns the WSL clipboard helper subprocess, used to pass a temporary image path. The variable is written into the PowerShell subprocess's environment and read by the subprocess script internally; the main `byf` process does not read this variable itself. Setting it in an external shell has **no effect** on the main `byf` process — users do not need to manage this variable manually.

## System environment variables

BYF also reads a handful of standard system environment variables to detect the runtime environment and pick default behavior:
...
These variables follow the usual conventions of each operating system; `byf` only reads them and never modifies them.
