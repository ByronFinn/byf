...
BYF stores its runtime data centrally under the `~/.byf/` directory in the user's home folder. This page describes where each type of data lives, what it is for, and how to customize or clean it up.

## Data root

The default data root is `~/.byf/`. The `~` is resolved by Node.js's `os.homedir()`, so the actual path differs slightly across platforms: on macOS it is `/Users/<name>/.byf`, on Linux `/home/<name>/.byf`, and on Windows `C:\Users\<name>\.byf`.

You can override it to any path with the `BYF_HOME` environment variable:

```sh
export BYF_HOME="$HOME/.config/byf"
```

Once set, runtime data such as the config, sessions, logs, input history, update cache, and OAuth credentials lands under that path. For the full reference on `BYF_HOME` and other environment variables, see [Environment variables](./env-vars.md).

::: tip Exceptions
The **built-in tool cache** (such as the auto-downloaded ripgrep binary) does not follow `BYF_HOME`. It uses `BYF_CACHE_DIR`, falling back to a platform cache directory — `~/Library/Caches/byf` on macOS, `$XDG_CACHE_HOME/byf` (default `~/.cache/byf`) on Linux, and `%LOCALAPPDATA%\byf` on Windows.

User-level Agent Skills search directories live at `~/.byf/skills` and `~/.agents/skills`; project-level Skills live under the working directory at `.byf/skills` and `.agents/skills`. See [Agent Skills](../customization/skills.md) for details.
:::
...
│   ├── <name>.json         # BYF provider OAuth credentials
...
::: tip
The tree above shows a typical layout under the default data root (`~/.byf/`). The paths for Agent Skills and the built-in tool cache have some special cases — see the "Exceptions" note above.
:::
...
- **BYF provider OAuth credentials** live at `credentials/<name>.json`, for example `~/.byf/credentials/managed:byf.json`.
...
- `agents/main/wire.jsonl`: the Wire event stream of the main agent, used for replay and resumption. `main` is the fixed id of the main agent. Replaying this journal rebuilds conversation history only; it does not replay tool calls, and side effects already written to local files or sent to remote services are not undone.
...
- `logs/byf.log`: the diagnostic log for this session. It only appears after a recorded diagnostic event; an ordinary conversation may not create this file.
...
The first time BYF needs ripgrep, it downloads and caches it automatically. During the download, the archive is written to the system temporary directory and verified by SHA-256 before extraction; the binary is then installed directly to `bin/rg` under the data root (or `bin/rg.exe` on Windows) and marked `0o755` so it can be executed. Subsequent runs under the same data root reuse it with no further download. If `rg` is already on the system `PATH`, the system version takes precedence; deleting `bin/` triggers a redownload the next time it is needed.

## Logs and update state

The top-level `logs/byf.log` is the global diagnostic log. It mainly records issues that do not belong to a single session, such as startup, login, and export failures. A single session's diagnostic log lives at `<sessionDir>/logs/byf.log`.

When filing a bug report, prefer `byf export` for the relevant session (see [The byf command](../reference/byf-command.md) for details). If a session log exists, it is included in the export by default. The global diagnostic log is also bundled by default; because it may contain events from other sessions or projects, use `--no-include-global-log` when you do not want to share it.
...
Deleting the data root directory (default `~/.byf/`, or the path specified by `BYF_HOME`) wipes all of BYF's runtime data, including config, sessions, logs, input history, and the built-in tool cache.
