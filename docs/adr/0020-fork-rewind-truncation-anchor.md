...
- **检查点语义（保留到所选 turn 结束）。** MVP 被拒绝：由于没有 `turn.end` wire 标记，"第 N 个 turn 的结束"只能近似为"就在第 (N+1) 个 `turn.prompt` 之前"——这对最后一个 turn（无后续锚点）和已取消的 turn 来说是脆弱的。编辑消息语义（在*第 N 个* `turn.prompt` 处截断）有明确的锚点。检查点语义推迟到 v2。
