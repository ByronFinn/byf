...
> **取代（ADR 0042）**：D4「回环默认体验不变（无 token）」不再成立——PRD-0038 R1 起，回环绑定同样每次启动自动生成一次性 token，所有写操作都要求它；只读 GET/HEAD 仅在「回环自动 token（未显式配置）」下免凭证。D1（LAN = token 持有者即全权）、D2（作用域文件端点）、D3（密钥只写不读）不受影响。
...
回环绑定继续默认无 token、行为不变；LAN 是显式 opt-in（`-H` + token）。（**取代注记**：本条已被 ADR 0042 推翻——回环绑定现每次启动自动生成一次性 token，写操作一律要求它；LAN 的 `-H` + token 语义不变。）
