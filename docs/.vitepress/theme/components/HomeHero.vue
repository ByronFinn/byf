...
<template>
  <section class="ByfHero">
    <div class="ByfHero__halo" aria-hidden="true" />
    <div class="ByfHero__inner">
      <div class="ByfHero__logo">
        <BYFLogo :size="64" />
      </div>
      <h1 class="ByfHero__title">
        <span class="ByfHero__accent">{{ copy.titleAccent }}</span>{{ copy.titleTail }}
      </h1>
      <p class="ByfHero__tagline">{{ copy.tagline }}</p>
      <div class="ByfHero__actions">
        <a class="ByfBtn ByfBtn--primary" :href="withBase(copy.primaryHref)">
          {{ copy.primaryText }}
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
            <path d="M6 3l5 5-5 5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
          </svg>
        </a>
        <a class="ByfBtn ByfBtn--ghost" :href="copy.secondaryHref" target="_blank" rel="noopener">
...
.ByfHero__accent {
  background: var(--byf-brand-gradient);
