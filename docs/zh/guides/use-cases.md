...
以下是 BYF 的典型使用场景和 prompt 示例。

## 理解陌生项目

接手陌生仓库时，直接告诉 Agent 先做调研再动手：
...
描述清楚需求和验收标准即可。复杂需求可让 Agent 先给出方案。
...
多文件重构建议让 Agent 先给出方案，可用 `/fork` 试探替代方案。
...
需要留档或复盘时，用 `byf export <sessionId>` 打包为 ZIP。
