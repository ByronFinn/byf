...
恢复或 fork 会话时重放的是事件日志，用来重建对话历史，并不代表会重放工具调用、也不会撤销工具已经产生的副作用：崩溃前写入的本机文件、以及已经发往远端的不可逆效果都会保持原样，崩在工具执行中的调用只会被记为一条未完成的观察，而不是一次回滚。
...
::: warning 注意
`sessions/` 目录下的文件手动修改后可能导致会话无法恢复，建议不要手工编辑。恢复是按 journal 重放来重建对话，并不是事务回滚，也不会撤销任何工具副作用。
:::
...
派生后的会话彼此独立，不影响原会话，你随时可以切回。fork 复制的只是会话历史，不会撤销任何已执行的工具效果；切回原会话也不代表本机文件或远端状态会退回从前。
