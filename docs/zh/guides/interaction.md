...
BYF 以交互式 TUI 运行在终端中，核心是一个输入框、对话视图和状态栏。本节介绍输入操作、模式切换、审批流程和常用快捷键，完整清单可通过 `/help` 查看。
...
部分命令仅在 Agent 空闲时可用，流式输出中需先中断才能调用。`/yolo`、`/help` 等模式切换类命令则始终可用。
...
Agent 调用可能产生副作用的工具（如修改文件、执行命令）时，会弹出审批面板让你确认。YOLO 模式除外。
