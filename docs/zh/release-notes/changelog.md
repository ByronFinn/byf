...
- 在首次启动迁移期间，将用户 Skill 从 `~/.byf/skills/` 迁移到 `~/.byf/skills/`；已存在的目标 Skill 会被保留。
- 在 stream-json 输出格式中以结构化 meta 消息形式发出会话恢复提示。

### 修复

- 在 OAuth 设备信息中改为上报 macOS 产品版本，而不是 Darwin 内核版本。
- 将 `X-Msh-Platform` 请求头的取值修正为 `byf_code_cli`。
...
- 默认让 BYF 请求使用剩余的上下文窗口作为补全 token 的额度，同时将显式设置的环境变量上限作为硬上限保留。
...
- 当未配置模型时，`/model` 和欢迎面板现在会引导用户使用 `/login`（针对 BYF）和 `/connect`（针对其他供应商）。
