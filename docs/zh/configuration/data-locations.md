...
BYF 将运行时数据集中存储在用户主目录下的 `~/.byf/` 目录中。本页介绍各类数据的存放位置、用途，以及如何自定义和清理。

## 数据根目录

默认数据根是 `~/.byf/`。`~` 由 Node.js 的 `os.homedir()` 解析，因此实际路径在不同平台略有差异：macOS 上是 `/Users/<name>/.byf`，Linux 上是 `/home/<name>/.byf`，Windows 上是 `C:\Users\<name>\.byf`。

可以通过 `BYF_HOME` 环境变量覆盖到任意路径：

```sh
export BYF_HOME="$HOME/.config/byf"
```

设置后，配置、会话、日志、输入历史、更新缓存、OAuth 凭据等运行时数据都会落到该路径下。`BYF_HOME` 与其他环境变量的完整说明见 [环境变量](./env-vars.md)。

::: tip 例外
**内置工具缓存**（例如自动下载的 ripgrep 二进制）不走 `BYF_HOME`，而是走 `BYF_CACHE_DIR`；未设置时使用平台缓存目录——macOS 上是 `~/Library/Caches/byf`，Linux 上是 `$XDG_CACHE_HOME/byf`（缺省 `~/.cache/byf`），Windows 上是 `%LOCALAPPDATA%\byf`。

用户级 Agent Skills 的搜索目录位于 `~/.byf/skills` 与 `~/.agents/skills`；项目级则是工作目录下的 `.byf/skills` 与 `.agents/skills`。详见 [Agent Skills](../customization/skills.md)。
:::
...
│   ├── <name>.json         # 托管 BYF 等 provider OAuth 凭据
...
::: tip
上面的目录树展示的是默认数据根（`~/.byf/`）下的典型布局。Agent Skills 与内置工具缓存的路径略有特殊性，详见上方"例外"提示。
:::
...
- **托管 BYF 等供应商的 OAuth 凭据**位于 `credentials/<name>.json`，例如 `~/.byf/credentials/managed:byf.json`。
...
BYF 在首次需要 ripgrep 时会自动下载并缓存。下载过程中，压缩包写入系统临时目录，校验 SHA-256 后解压，二进制直接安装到数据根下的 `bin/rg`（Windows 上为 `bin/rg.exe`）并赋予 `0o755` 执行权限。后续在同一数据根下直接复用，无需再次下载。如果系统 `PATH` 中本来就有 `rg`，会优先使用系统版本；删除 `bin/` 会在下一次需要时触发重新下载。

## 日志与更新状态

顶层 `logs/byf.log` 是全局诊断日志，主要记录启动、登录、导出等不属于单个会话的问题。单个会话自己的诊断日志在 `<sessionDir>/logs/byf.log`。

如需报告 bug，优先使用 `byf export` 导出相关会话（详见 [byf 命令](../reference/byf-command.md)）；如果会话日志存在，它会默认包含在导出包里。全局诊断日志默认也会打包；因为它可能包含其它会话或其它项目的事件，不想分享时使用 `--no-include-global-log` 排除。
...
直接删除数据根目录（默认 `~/.byf/`，或 `BYF_HOME` 指定的路径）可以完全清理 BYF 的所有运行时数据，包括配置、会话、日志、输入历史和内置工具缓存。

如只需清理部分数据：

| 需求 | 操作 |
| --- | --- |
| 重置配置 | 删除 `~/.byf/config.toml` |
| 清理所有会话 | 删除 `~/.byf/sessions/` 与 `~/.byf/session_index.jsonl` |
| 清理诊断日志 | 删除 `~/.byf/logs/` 目录 |
| 清理输入历史 | 删除 `~/.byf/user-history/` 目录 |
| 重置更新检查状态 | 删除 `~/.byf/updates/latest.json` |
| 强制重新下载 ripgrep | 删除 `~/.byf/bin/` 目录 |
| 清除托管 BYF OAuth 登录态 | 运行 `/logout`（仅清理当前供应商的 OAuth），或删除对应 `~/.byf/credentials/<name>.json` |
| 清除 MCP server OAuth 登录态 | 删除 `~/.byf/credentials/mcp/` 目录；`/logout` **不会**清理 MCP 的 OAuth 凭据 |
| 移除用户级 MCP 声明 | 删除 `~/.byf/mcp.json` |
| 清空用户级 Skills | 删除 `~/.byf/skills/` 目录 |
