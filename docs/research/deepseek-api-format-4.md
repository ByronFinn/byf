...
- **出站回传形态已实测（2026-08-13，无缺口）**：多轮工具调用场景下，把 reasoning 分别以 **summary 形态**（`{type:'reasoning', summary:[{type:'summary_text', text}]}`，byf 当前 `convertMessage` 做法）和 **content 形态**（`{type:'reasoning', content:[{type:'reasoning_text', text}]}`，DeepSeek 原生输出形态）回传给 DeepSeek Responses，**两者均 HTTP 200 被接受**，模型正常产生新一轮 reasoning + 回答。故 byf 的 summary 形态回传安全，无需改动；DeepSeek Responses 输入侧对两种 reasoning 形态兼容。
