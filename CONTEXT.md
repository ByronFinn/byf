...
**两类 record**：(1) 可恢复 record——restore 时由各子系统的 `RecordRestoreHandler.restoreRecord` 重建状态；(2) live-only 调试 record——仅记录 live 时发生的瞬时优化（如 `context.output_offloaded`/`context.pruning`），restore 跳过，仅供 vis wire 视图展示。后者在 `restoreRecord` 的 switch 里是**有意缺席**，非 bug。

### wire 折叠 (wire fold) / 投影函数 (projection function)

把 wire record / loop event 流重建为 `ContextMessage[]` 时间线的过程称为 **wire 折叠**。内核的 `ContextMemory.appendLoopEvent` 是折叠实现，但它和落盘/offload/通知等副作用耦合。**投影函数**特指从折叠逻辑中剥离出的纯函数（`replayWireToHistory(records): ContextMessage[]`），无副作用，供 vis 复用以消除重复。区别于 `agent/context/projector.ts` 的 `project()`——后者是「已折叠 history → provider 请求体」的投影，是另一层（见「投影 (Project)」）。
...
**Live-only 语义**：offload 是 live 时基于当前 token 压力的临时优化，restore 路径有意跳过它（恢复后的 `_history` 携带原始完整输出，下次 turn 的 `beforeStep` 会重做压缩）。临时文件易失不可恢复。`context.output_offloaded` wire record 写入仅作 vis 调试徽章，不参与 restore。`context.pruning` 同理——记录 live 裁剪事件，restore 重建原始未裁剪内容后由 `beforeStep` 重做。
