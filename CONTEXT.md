...
按来源类别拆分的输入 token 估算占比，用于回答"上下文被什么占满"。六个互斥类别：System prompt（核心规则块）、Meta context（项目指令 + 工作环境块）、Skills（技能列表块）、MCP tools / System tools（按工具名前缀拆分的工具 schema）、Messages（历史消息）。**估算值**（char 启发式，provider 不返回按类别的精确 token），分母为六项估算之和；百分比经最大余数法归一化，严格相加 = 100%。与 "Context window" 行的 provider 真实 token 不直接挂钩。仅在 `/usage` 面板按需展示。
