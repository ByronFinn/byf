/**
 * 轨迹时间轴总览（Chrome-Network 式），移植自 deepseek-harness
 * `ui-trajectory/src/client/TrajectoryTimeline.tsx`（MIT，交互与视觉对齐，
 * token 换用 BYF 语义色）。渲染在检视-轨迹（WireTab）顶部，作为扁平 wire
 * 账本的时间/序号总览。
 *
 * 交互契约（与 deepseek 一致）：
 * - 左键拖选 = 聚焦区间：账本中区间内记录保留亮度、区间外变暗（onRangeChange
 *   向上传递，WireTab 据此 dim 行）；
 * - 点击 span = 选中并清区间（onRecordSelect，跳转该 wire 行）；
 * - 点击空白 = 聚焦最近 span（onRecordFocus，滚动到该行）；
 * - 滚轮 = 以指针为锚缩放（sequence 模式最小 4 个 span，其他模式 20）；
 * - 右键/中键拖动 = 平移视口（仅已缩放时）；双击 = 重置；Esc = 清区间；
 * - 悬停 = 垂直参考线 + span 高亮 + 500ms 延迟 tooltip（时长/TTFT/墙钟）。
 *
 * 数据派生在 `lib/timeline.ts`（deriveTimeline 纯函数），本组件只渲染与交互。
 */
import {
  memo,
  useEffect,
...
function timelineKindLabel(kind: TimelineCellKind): string {
  switch (kind) {
    case 'system':
      return '系统';
    case 'user':
...
/**
 * tooltip 用 CSS `::after` 伪元素承载（`data-tooltip` 属性 + `attr()`），
 * 不包裹子元素 —— 包裹会破坏 span 的绝对定位（span 相对 `.lanes` 定位，
 * 任何 relative/inline 包装器都会让它堆叠在左缘）。hover 延迟由
 * `.span` 的 transition-delay 近似（500ms 显示、即时隐藏）。
 */
function timelineTooltipAttr(label: string): string {
...
export const TrajectoryTimeline = memo(function TrajectoryTimeline({
  ...
  range,
  selectedLineNo = null,
  searchMatchLineNos = null,
  onRangeChange,
  onRecordSelect,
  onRecordFocus,
}: TrajectoryTimelineProps) {
  const model = useMemo(() => deriveTimeline(turns, mode), [mode, turns]);
  const detailByIndex = useMemo(
    () =>
      new Map(
        turns.flatMap((turn) =>
          turn.cells.map((cell) => [cell.index, timelineRecordDetail(cell)] as const),
        ),
      ),
    [turns],
  );
  ...
  useEffect(() => {
    if (model === null || selectedLineNo === null) return;
    const selectedSpan = model.spans.find((span) => span.lineNo === selectedLineNo);
      ...
      <section ref={rootRef} className={css['root']} aria-label="Trajectory timeline">
        <div className={css['plot']}>
          ...
          <div className={css['track']}>
            <span className={css['empty']}>暂无时间数据</span>
          </div>
        </div>
      </section>
    );
  ...
  const commit = (nextRange: TimelineRange) => {
  ...
  const onPointerEnd = (event: PointerEvent<HTMLDivElement>) => {
    ...
    if (clickedSpan !== undefined) {
      onRangeChange(null);
      onRecordSelect?.(clickedSpan.lineNo);
      return;
    }
              ...
              .map((span) => {
                ...
                return (
                  <span
                    ...
                    data-tooltip={timelineTooltipAttr(timelineTooltipLabel(span.kind, detail))}
                    ...
                    data-current={span.lineNo === selectedLineNo || undefined}
                    data-hovered={hover?.recordIndex === span.index || undefined}
                    data-search-match={
                      searchMatchLineNos === null
                        ? undefined
                        : searchMatchLineNos.has(span.lineNo)
                          ? 'true'
                          : 'false'
                    }
                    data-selected={
