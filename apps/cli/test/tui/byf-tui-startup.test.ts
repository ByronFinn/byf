...
function loginRequiredError(): Error & { readonly code: string } {
  return Object.assign(new Error('OAuth provider "managed:byf" requires login.'), {
...
describe("ByfTui startup", () => {
  ...
  it.skip("tracks login with already_logged_in when a token already exists", async () => {
    ...
    const harness = makeHarness(session, {
      auth: {
        status: vi.fn(async () => ({
          providers: [{ providerName: "managed:byf", hasToken: true }],
        })),
        login: vi.fn(async () => {}),
        logout: vi.fn(),
        getManagedUsage: vi.fn(),
  ...
  it.skip("logs login failures with session context", async () => {
    ...
    const session = makeSession();
    const loginError = new Error("Failed to list Byf Code models (HTTP 402).");
    const harness = makeHarness(session, {
      auth: {
        status: vi.fn(async () => ({ providers: [] })),
        login: vi.fn(async () => {
    ...
    try {
      ...
      expect(warn).toHaveBeenCalledWith(
        "login failed",
        expect.objectContaining({
          providerName: "managed:byf",
          alreadyLoggedIn: false,
          sessionId: "ses-1",
          error: expect.objectContaining({
            message: "Failed to list Byf Code models (HTTP 402).",
          }),
        }),
      );
    } finally {
  ...
  it.skip("tracks logout after managed credentials and session state are cleared", async () => {
    ...
    const harness = makeHarness(session, {
      getConfig: vi.fn(async () => ({
        models: {
          k2: { provider: "managed:byf", model: "byf-v1", maxContextSize: 100 },
        },
        providers: { "managed:byf": { type: "byf" } },
      })),
      auth: {
        status: vi.fn(async () => ({
          providers: [{ providerName: "managed:byf", hasToken: true }],
    ...
    expect(harness.auth.logout).toHaveBeenCalledWith("managed:byf");
  ...
  it.skip("can log out a stale managed entry even after the OAuth token is gone", async () => {
    ...
    const harness = makeHarness(session, {
      ...
      auth: {
        ...
        status: vi.fn(async () => ({
          providers: [{ providerName: "managed:byf", hasToken: false }],
        })),
        login: vi.fn(async () => {}),
        logout: vi.fn(),
        getManagedUsage: vi.fn(),
