...
describe('fetchLatestVersionFromGitHub', () => {
  it('returns the trimmed semver returned by GitHub Releases /latest', async () => {
    ...
    expect(fetchLatestVersionFromGitHub(f)).resolves.toBe('0.5.0');
    expect(f).toHaveBeenCalledWith(BYF_RELEASES_LATEST_URL);
  });

  it('throws when response is non-2xx', async () => {
    expect(fetchLatestVersionFromGitHub(mockFetchStatus(404))).rejects.toThrow(/HTTP 404/);
  });

  it('throws when body is not valid semver', async () => {
    expect(fetchLatestVersionFromGitHub(mockFetchOk('not-a-version'))).rejects.toThrow(
      /invalid semver/,
    );
  });

  it('throws when body is empty', async () => {
    expect(fetchLatestVersionFromGitHub(mockFetchOk('   '))).rejects.toThrow(/invalid semver/);
  });
  ...
  it('propagates the underlying fetch error', async () => {
    ...
    expect(fetchLatestVersionFromGitHub(f)).rejects.toThrow(/network down/);
  });
});
