...
import { fetchLatestVersionFromGitHub } from './cdn';
...
export interface RefreshUpdateCacheDeps {
   ...
   * Errors intentionally skip `writeCache` so a transient GitHub Releases API
   * blip does not overwrite a previously known `latest` with `null`. */
...
export async function refreshUpdateCache(
  ...
  const resolved: RefreshUpdateCacheDeps = {
    fetchLatest: overrides.fetchLatest ?? (() => fetchLatestVersionFromGitHub()),
