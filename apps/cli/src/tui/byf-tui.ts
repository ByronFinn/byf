...
function createInitialAppState(input: ByfTuiStartupInput): AppState {
...
// Creates all pi-tui components and mutable runtime state owned by ByfTui.
export function createTUIState(options: ByfTuiOptions): TUIState {
...
export class ByfTui implements DialogHost {
  ...
  constructor(harness: ByfHarness, startupInput: ByfTuiStartupInput) {
    this.harness = harness;
    const tuiOptions: ByfTuiOptions = {
  ...
  private getSlashCommands(): readonly ByfSlashCommand[] {
  ...
  private applyTheme(theme: Theme, resolved?: ResolvedTheme): void {
    const nextTheme = createByfTuiThemeBundle(theme, resolved);
